...
import './EdtContainerAcceuil.css';
...
const EdtContainerAcceuil = () => {
    ...
    const handlePrintSingle = (niveauId: string, parcoursId: string) => {
        ...
        // Filtrer les EDT par niveau, parcours et date sélectionnée
        ...
        if (edtsForNiveauAndParcours.length > 0) {
            // Trouver le créneau correspondant à la date sélectionnée ou le plus récent
            const latestEdt = edtsForNiveauAndParcours.reduce((latest, current) => {
    ...
    const handlePrintAll = () => {
        ...
        let printContent = `
            <html>
                <head>
                    <title class="title">Emplois du temps - ${selectedDateDebut || getLatestDateDebut()}</title>
                    <style>
                        body { font-family: Arial, sans-serif; margin: 20px; }
                        .title{ text-align: right; position: absolute; right: 0; }
                        .print-header { text-align: center; margin-bottom: 20px; }
                ...
                <body>
                    <div class="print-header">
                        <div class="print-title">Emplois du temps</div>
        ...
        niveaux.forEach(niveau => {
            parcours.forEach(parcour => {
                if (hasEdtsForNiveauAndParcours(niveau.IDNiveaux, parcour.IDParcours)) {
                    const niveauDetails = getNiveauDetails(niveau.IDNiveaux);
                    const parcoursDetails = getParcoursDetails(parcour.IDParcours);
                    
                    // Filtrer les EDT par niveau, parcours et date sélectionnée
                    let edtsForNiveauAndParcours = edts.filter(edt => 
                        edt.IDNiveaux === niveau.IDNiveaux && edt.IDParcours === parcour.IDParcours
                    );
                    ...
                    creneauxGroupes.forEach(({ heureDebut, heureFin, creneauxIds }) => {
                        ...
                        ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi'].forEach(jour => {
                            let edtForJour = null;
                            for (const creneauId of creneauxIds) {
                                edtForJour = getEdtForCreneauAndDay(niveau.IDNiveaux, parcour.IDParcours, creneauId, jour);
                                if (edtForJour) break;
                            }
                    ...
                    <form>
                        ...
                        <div className="edt-buttons">
                            <Button 
                                ...
                                onClick={() => handlePrintSingle(niveauId, parcoursId)}
                            >
                                <ImPrinter /> &nbsp; IMPRIMER
                            </Button>
                        </div>
...
export default EdtContainerAcceuil;